var popIframe = {
  scrolling: 'auto',
  //是否显示滚动条 no,yes,auto
  el: null,
  closeFn: function(){},
  init: function () {
    this.mouseClose();
    this.closeMask();
    //this.mouseDown();
  },

  showWin: function (id, src, closeFn, width, height, title) {
    this.el = $(id);
    this.el.show();
    width = width || "750";
    height = height || "530";
    title = title || "";
    var iframeHeight = height - 32;
    // var marginLeft = width / 2;
    // var marginTop = height / 2;
    var inntHtml = '';
    // inntHtml += '<div id="mask" style="width:100%; height:100%; position:fixed; top:0; left:0; z-inde:1999;background:#cccccc; filter:alpha(opacity=50); -moz-opacity:0.5; -khtml-opacity: 0.5; opacity:0.5;"></div>'
    inntHtml += '<div id="maskTop" style="width: ' + width + 'px; height: ' + height + 'px; border: #999999 1px solid; background: #fff; color: #333; z-index: 2999; filter: progid:DXImageTransform.Microsoft.Shadow(color=#909090,direction=120,strength=4); -moz-box-shadow: 2px 2px 10px #909090; -webkit-box-shadow: 2px 2px 10px #909090; box-shadow: 2px 2px 10px #909090;">'
    inntHtml += '<div id="maskTitle" style="height: 30px; line-height: 30px; font-family: Microsoft Yahei; font-size: 16px; color: #333333; padding-left: 20px; background-image: url(data:image/png;base64,); border-bottom: 1px solid #999999; position: relative;">'
    inntHtml += '' + title + ''
    inntHtml += '<div id="popIframeClose" style="width: 28px; height: 28px; cursor: pointer; position: absolute; top: -12px; right: -9px; background-image: url(data:image/png;base64,);"></div>'
    inntHtml += '</div>'
    inntHtml += '<iframe width="' + width + '" height="' + iframeHeight + '" frameborder="0" scrolling="' + this.scrolling + '" src="' + src + '"></iframe>';

    this.el.html(inntHtml);
    this.init();
    closeFn && (this.closeFn = closeFn)
  },

  mouseClose: function () {
    $("#popIframeClose").on('mouseenter',
      function () {
        $(this).css("background-image", "url(data:image/png;base64,)");
      });

    $("#popIframeClose").on('mouseleave',
      function () {
        $(this).css("background-image", "url(data:image/png;base64,)");
      });
  },

  closeFadeOut: function(){
    if($("#mask,#maskTop").length > 0){
      var that = this;
      $("#mask,#maskTop").fadeOut(function () {
        $(this).remove();
        that.el.hide();
      });
    }
  },

  closeMask: function () {
    var that = this;
    $("#popIframeClose").on('click',
      function () {
        that.closeFn();
        that.closeFadeOut();
      }
    );
  }

  /*mouseDown : function(){
      var dragging = false;
      var iX, iY;
      //var elmen = $("div#maskTop");
      $("#maskTop").on('mousedown' , function(e){
        dragging = true;
                  iX = e.clientX - this.offsetLeft;
                  iY = e.clientY - this.offsetTop;
                  this.setCapture && this.setCapture();
                  return false;
      });
      document.onmousemove = function(e) {
                  if (dragging) {
                  var e = e || window.event;
                  var oX = e.clientX - iX;
                  var oY = e.clientY - iY;
                  $("#maskTop").css({"left":oX + "px", "top":oY + "px"});
                  return false;
                  }
              };
              $(document).mouseup(function(e) {
                  dragging = false;
                  $("#maskTop")[0].releaseCapture();
                  e.cancelBubble = true;
              })
    },*/

};