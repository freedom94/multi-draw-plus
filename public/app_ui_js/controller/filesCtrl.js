...
fileListModule.controller('FileListCtrl', function($scope, $http) {
    ...
    $scope.loadAllParticipantsFiles = function () {
        $http.get("/loadAllParticipantsFiles?isRecycled=false")
            .success(function (res) {
                if(res.success) $scope.files = res.data;
            });
    };
    ...
    $scope.loadWatch = function (file,isReload) {
        console.log(isReload);
        var queryObj = {};
        if(this.getSelectedFilesIds())
          queryObj.id = this.getSelectedFilesIds()[0];
        if(file)
          queryObj.id = file.id;
        if(!isReload)
          window.open('/watch' + mdUtils.convertJSONToQueryStr(queryObj,true));
        else
          window.location.href= '/watch' + mdUtils.convertJSONToQueryStr(queryObj,true);
    };
